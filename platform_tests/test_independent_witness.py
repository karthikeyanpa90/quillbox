...
from agent import DEF, MEASURES, NotIndependent, QuillAgent          # noqa: E402
...
def test_the_witness_job_delivers_every_measure_signed_and_independent(owl, tmp_path):
    base, svc = owl
    ...
    for m in MEASURES:
        ...
        assert rows[0]["about"]["build_version"] == "v1"
...
def test_the_reading_carries_a_digest_of_the_exact_source_measured(owl, tmp_path):
    ...
    about = c.get(f"/v1/systems/{keys['id']}/records/coverage?week=1", headers=owner).json()[0]["about"]
    assert about["code"] == tree_digest(QUILLBOX / "candidates" / "v1" / "quillbox_app")
    assert about["suite"] == tree_digest(QUILLBOX / "tests")

def test_the_numbers_are_the_tools_own_and_differ_between_real_candidates(owl, tmp_path):
    """v1 builds the compose story that v0 does not: a real difference, from pytest's real report."""
    base, svc = owl
    c, keys, owner, private = make_system(base, svc)
    run_witness(base, keys["id"], private, "v0", 1, tmp_path)
    run_witness(base, keys["id"], private, "v1", 2, tmp_path)
    def story(week):
        return c.get(f"/v1/systems/{keys['id']}/records/story_completion?week={week}", headers=owner).json()[0]["value"]
    assert story(2) > story(1)
...
def test_the_witness_never_asks_the_graded_application_anything():
    ...
    assert used <= {"QB_SID", "QB_OWL_BASE_URL", "QB_VERSION", "QB_INDEX", "QB_WITNESS_KEY_FILE",
                    "QB_IMAGE", "QB_SANDBOX_USER"}, f"the witness reads {used}"
    clients = [n for n in ast.walk(tree) if isinstance(n, ast.Call)
               and isinstance(n.func, ast.Attribute) and n.func.attr == "Client"]
    assert len(clients) == 1
    assert [k.value.id for k in clients[0].keywords if k.arg == "base_url"] == ["owl_base"]
...
def test_quill_agent_refuses_a_reading_the_tenant_delivered(owl, tmp_path):
    """story_tests_total is in neither guards nor goal, so OWL permits it to be declared tenant-delivered. Quill
    Agent still refuses to decide on it: 'shown, never judged' has to mean something on the deciding side too."""
    base, svc = owl
    ...
    run_witness(base, keys["id"], private, "v1", 3, tmp_path)
...
def test_a_real_candidate_is_applied_measured_and_promoted(owl, quillbox, tmp_path):
    """v0 is live; v1 builds the compose story. The adapter applies for real, the witness measures for real in a
    separate process with its own keys, OWL holds both readings, and Quill Agent promotes on what OWL holds."""
    base, svc = owl
    ...
    assert out["candidate"]["story_completion"] > out["current"]["story_completion"]
    assert httpx.get(f"{qb_base}/status", timeout=30).json()["live_version"] == "v1"

    defn = c.get(f"/v1/systems/{keys['id']}/definition", headers=owner).json()
    assert defn["levers"][0]["options"] == ["v1"] and defn["levers"][0]["start"] == "v1"
    entry = [e for e in c.get(f"/v1/systems/{keys['id']}/ledger", headers=owner).json()
             if e["body"].get("variable") == "definition"][-1]
    assert entry["body"]["version0"] == {"build_version": "v1"}       # the promotion is legible on the ledger
    assert c.get(f"/v1/systems/{keys['id']}/ledger/verify", headers=owner).json()["ok"] is True

def test_a_candidate_that_shrinks_the_acceptance_suite_is_refused(owl, quillbox, tmp_path):
    """The cheapest way to raise a share is to shrink its denominator. OWL is not asked to judge the suite's
    size, so Quill Agent refuses it here -- on numbers OWL holds, delivered by the same independent witness."""
    base, svc = owl
    qb_base, secret, root = quillbox
    c, keys, owner, private = make_system(base, svc)
    agent = QuillAgent(httpx.Client(base_url=base, timeout=60.0), sid=keys["id"], owner_key=keys["owner_key"],
                       runtime_key=keys["runtime_key"], qb=httpx.Client(base_url=qb_base, timeout=60.0),
                       adapter_secret=secret, witness_runner=lambda v, i: None)
    readings = {"v0": {"compliance_pass": 1.0, "critical_defects": 0, "coverage": 0.8, "story_completion": 0.33,
                       "story_tests_total": 3},
                "v1": {"compliance_pass": 1.0, "critical_defects": 0, "coverage": 0.9, "story_completion": 1.0,
                       "story_tests_total": 1}}
    agent.measure = lambda version, index: readings[version]
    out = agent.promote_if_better("v1", "v0", 1)
    assert out["promoted"] is False and "acceptance suite shrank" in out["reason"]
    assert c.get(f"/v1/systems/{keys['id']}/definition", headers=owner).json()["levers"][0]["options"] == ["v0"]
