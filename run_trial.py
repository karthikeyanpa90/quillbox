# -*- coding: utf-8 -*-
"""Run real weeks now that the adapter is live: Quillbox's own /run-week reports whatever's
currently applied, OWL's /cycle judges and, with a second build_version option now available,
may apply v1 as a real trial. Reads all keys from .deploy-env.yaml -- nothing hardcoded.

Usage:  C:\\acresgo\\quillbox\\.venv\\Scripts\\python.exe run_trial.py [n_weeks]
"""
import json
import sys
...
if __name__ == "__main__":
    n_weeks = int(sys.argv[1]) if len(sys.argv) > 1 else 5
    env = _read_env_file()
    ...
    start_week = (st0.get("week") or 0) + 1
    print(f"starting from week {start_week}, current live x: {st0.get('x')}, counts: {st0.get('counts')}")

    for w in range(start_week, start_week + n_weeks):
        rw = qb.post(f"/run-week/{w}", headers=driver_hdr)
        rw.raise_for_status()
        rc = owl.post(f"/v1/systems/{sid}/cycle/{w}", headers=runtime_hdr)
        rc.raise_for_status()
        qst = qb.get("/status").json()
        print(f"week {w}: report={rw.json()}  cycle={rc.json()}  quillbox_live_version={qst['live_version']}")
